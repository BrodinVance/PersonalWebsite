...
## The Experiment:
As part of the introduction, we must introduce the very experiment that will be used to rule out Local Realism. This experiment involves everyone's favourite duo: Alice and Bob, along with an electron emitter and two Stern-Gerlach magnets. The introduction will explain and show, with a visual, how the electron emitter will be set up to fire two electrons, one into Alice's detector and one into Bob's. Before the experiment, Alice and Bob will each choose two axis angles; the magnets will flip randomly between the two angles with each electron-emitter pulse. Each detector independently measures the electron's spin along the chosen axis. If they measure spin up, they will output +1; if they measure spin down, they will output -1. Ultimately, Alice and Bob's goal is to create the scalar $S$, based on expectation values after running the experiment many times, which we will define more rigorously in a later section.
